...
static void report_dot(const char *name, timespec_t start_time, size_t flops, double result) {
  timespec_t current_time;
  get_current_time(&current_time);
  double time_taken = time_delta(start_time, current_time);
  double rate = flops / time_taken;
  if (fabs(result - flops) > 1e-10)
    printf("Result %f failed to validate with expected value %ld\n", result, flops);
  printf("%s\t%ld\t%8.8g\t%8.8g\t\n", name, flops, time_taken, rate / 1E6);
}
...
#define REPORT_DOT(f) do {                                              \
    for (int rep=0; rep<args.nreps; rep++) {                            \
      ...
      report_dot(#f, start_time, 2*args.length, f(args.length, a, b)); \
    }                                                                   \
  } while (0)

static void report_bdot(const char *name, timespec_t start_time, size_t flops,
...
#define REPORT_BDOT(f, c, jstride, kstride, c_ref) do {                 \
    for (int rep=0; rep<args.nreps; rep++) {                            \
      ...
      get_current_time(&start_time);                                    \
      f(args.length, a, b, c);                                          \
      report_bdot(#f, start_time, 2*J*K*args.length, c, jstride, kstride, c_ref); \
    }                                                                   \
  } while (0)


// --- Actual computation code (where you will experiment with implementation)
...
// blocksize, how many total sums to keep track of at once
#define BS 16
