# HW-1: Experiments in Vectorization

## Part 2 (a prediction) due 2026-09-11 (Friday), the rest due approximately 2026-09-17 (next Thursday)

- No need to tag me for the prediction, just make sure you do it before 

## Dot products

We begin by computing a dot product of vectors
...
The first cannot be overcome without fusing with surrounding context (so the dot product is performed while memory is in cache for other reasons).
But the second can be addressed by techniques such as the following, which relax data dependencies by reordering operations (in this case, summing the even- and odd-indexed values separately.).
